...
public class PreferencesSettingsManager implements SettingsManager {
    ...
    private void loadConnectionsInfo() {
        ...
        for (int i = 0; i < connectionsCount; i++) {
            ...
            AppServerConnectionInfo appServer = new AppServerConnectionInfo();
            appServer.setHost(preferences.get("appServer." + name + ".host", DEFAULT_STRING_VALUE));
            appServer.setPort(preferences.get("appServer." + name + ".port", DEFAULT_STRING_VALUE));
            appServer.setUsername(preferences.get("appServer." + name + ".username", DEFAULT_STRING_VALUE));
            appServer.setPassword(preferences.get("appServer." + name + ".password", DEFAULT_STRING_VALUE));
            connectionInfo.setAppServerConnectionInfo(appServer);
    ...
    private void saveConnectionsInfo() {
        ...
        for(ConnectionInfo connectionInfo : connectionsInfo) {
            ...
            AppServerConnectionInfo appServer = connectionInfo.getAppServerConnectionInfo();
            preferences.put("appServer." + name + ".host", appServer.getHost().trim());
            preferences.put("appServer." + name + ".port", appServer.getPort().trim());
            preferences.put("appServer." + name + ".username", appServer.getUsername().trim());
            preferences.put("appServer." + name + ".password", appServer.getPassword().trim());
